...
# The piece array
var possible_pieces = [
	preload("res://scenes/Piece/yellow_piece.tscn"),
...
func match_at(i, j, color):
	if i > 1:
		if all_pieces[i-1][j] != null && all_pieces[i-2][j] != null:
			if all_pieces[i-1][j].color == color && all_pieces[i-2][j].color:
				return true
	if j > 1:
		if all_pieces[i][j-1] != null && all_pieces[i][j-2] != null:
			if all_pieces[i][j-1].color == color && all_pieces[i][j-2].color:
				return true
	return false
...
func _on_destroy_timer_timeout():
	print ("on destroy timer timeout")
	destroy_matched()


func _on_collapse_timer_timeout():
	print ("on collapse timer timeout")
	collaspe_columns()


func _on_refill_timer_timeout():
	print ("on refill timer timeout")
	refill_columns()